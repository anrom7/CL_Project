...
with codecs.open("C:/CL_Project/VI/Rurych_seedwords.txt", 'r', encoding='utf-8') as seed_file: 
	 ...
	 seed_list = [x.replace("\r\n", "") for x in seed_list]
	 print("Seed_words count {count}".format(count=len(seed_list))) #âèâ³ä çàãàëüíî¿ ê³ëüêîñò³ ñë³â Seed_words 


with codecs.open("C:/CL_Project/VI/Rurych_dic.txt", 'r', encoding='utf-8') as dic_file:
	dic_list = dic_file.readlines()
	dic_list = [x.replace("\r\n", "") for x in dic_list]
	print("Dic_words count {count}".format(count=len(dic_list)))#âèâ³ä çàãàëüíî¿ ê³ëüêîñò³ ñë³â dic_words
tp=[]
tp1=[]
tp2=[]
tp3=[]
...
tp=len(tp1)+len(tp2)+len(tp3) #ê³ëüê³ñòü TP
seed=''.join(seed_list)
seed_len=len(seed)
...
#ï³äðàõóíîê ñë³â ó çãåíåðîâàíîìó ñëîâíèêó, ÿê³ íåïðàâèëüíî ïðîìàðêîâàí³
incorrect_dic=[]
with codecs.open("C:/CL_Project/VI/Rurych_dic.txt", 'r', encoding='utf-8') as dic_file:
    for j in dic_file.readlines():
        d=j.split(',')
